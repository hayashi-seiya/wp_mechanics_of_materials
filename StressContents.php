<h2>応力とひずみ</h2>
    ...
    <h3>応力</h3>
        ...
        <p>垂直応力はよく<?php getInlineTex("\sigma")?>で表され、その求め方は、
            ...
            <?php getInlineTex("P")?>が正なら引張力、負なら圧縮力である。
        </p>
        <p>
            垂直応力とは、断面に生じる単位面積あたりの内力の、
    ...
    <h3>縦ひずみと横ひずみ</h3>
        <p>
            また荷重の種類によって<strong>引張ひずみ</strong>・<strong>圧縮ひずみ</strong>に分類され、
            荷重方向のひずみをまとめて<strong>垂直ひずみ</strong>という。
            通常の材料であれば、材料に引張(圧縮)荷重が作用した場合、対象物の断面積は収縮(増大)する。</p>
        <p>
            このように、荷重に垂直な方向のひずみを<strong>横ひずみ</strong>といい、<?php getInlineTex("\\varepsilon'")?>で表すと
            <?php getTex("\\varepsilon'=\\frac{d_1-d}{d}")?>
            となる。
        </p>
        <p>
            これに対して、荷重方向のひずみをを<strong>縦ひずみ</strong>という。これは前述の垂直ひずみと同義である。
        </p>
    <h3>ポアソン比</h3>
        <p>
            縦ひずみに対する横ひずみの比を<?php getInlineTex("\\nu")?>で表しポアソン比と呼ぶ。
            <?php getTex("\\nu=-\\frac{\\varepsilon'}{\\varepsilon}")?>
        </p>
        <p>
            ポアソン比がマイナスで表現されているのは、
            ...
            横ひずみ<?php getInlineTex("\\varepsilon'")?>は正であるためである。
        </p>
        <p>
            多くの材料のポアソン比は<?php getInlineTex("0.25〜0.35")?>である。
        </p>
